// hooks/useAIManagement/useTemplateEditor.ts
...
export function useTemplateEditor({
    ...
    useEffect(() => {
        if (initialTemplate && initialTemplate.trim() !== "") {
            try {
                const obj = parseTemplateToObject(initialTemplate);
                if (Object.keys(obj).length > 0) {
                    setTemplateObject(obj);
                    setTemplateString(objectToString(obj));
                    setIsValid(true);
                } else {
                    setTemplateString(initialTemplate);
                    setTemplateObject({});
                    setIsValid(false);
                }
            } catch (e: any) {
    ...
    }, [initialTemplate, parseTemplateToObject, objectToString]);
